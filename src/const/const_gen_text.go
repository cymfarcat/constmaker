...
func (obj *ConstParser) GenText(option *src.Options) {
	builder := new(strings.Builder)

	oldEnumToPrefix := option.EnumToPrefix
	oldNamespaceToPrefix := option.NamespaceToPrefix
	option.EnumToPrefix = false     // Text no enum
	option.NamespaceToPrefix = true // Text no namespace

	// generate root
	option.Level = 0
	obj.NameSpace.GenText(option, builder)

	path := option.GetOutputFile("txt")
	data := builder.String()
...
func textTypo(typo string) string {
	switch typo {
	case src.Typo_Bool:
		return "bool"
...
func (obj *ConstObject) GenText(prefix string, option *src.Options, builder *strings.Builder) {
	for idx, constObj := range obj.Children {
		...
		if constObj.objType == TypeEnumValue && !(obj.EnumToPrefix || option.EnumToPrefix) {
			if obj.enumSetValue {
				str = "\n" + option.GetTabWidth() + genIdentName(option, prefix, constObj) + ": " + textTypo(constObj.Typo) + " = " + constObj.Value
			} else {
				str = "\n" + option.GetTabWidth() + genIdentName(option, prefix, constObj) + ": " + textTypo(constObj.Typo) + " = " + constObj.Value
			}
		} else {
...
func (obj *ConstObject) genTextStr(prefix string, option *src.Options, builder *strings.Builder) {
...
func (obj *ConstObject) genTextId(prefix string, option *src.Options, builder *strings.Builder) {
	for _, constObj := range obj.genObjs {
...
/*
 * Text no namespace and enum
 */
func (obj *NameSpace) GenText(option *src.Options, builder *strings.Builder) {
	namespace := genIdentName(option, "", &obj.ConstObject)
	...
	obj.ConstObject.GenText(option.GetPrefixes(""), option, builder)

	// namespace need gen id
	if len(obj.genObjs) > 0 {
		obj.genTextId(option.GetPrefixes(""), option, builder)
	}
	...
	for _, enumObj := range obj.Enums {
		...
		if /*enumObj.EnumToPrefix ||*/ option.EnumToPrefix {
			...
			enumObj.GenText(enumName, option, builder)
		}
		...
		if len(enumObj.genObjs) > 0 {
			...
			enumObj.genTextStr(option.GetPrefixes(""), option, builder)
			option.PopPrefixes()
		}
	...
	for _, namespace := range obj.Namespaces {
		namespace.GenText(option, builder)
	}
