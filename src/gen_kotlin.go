...
func (obj *ConstParser) genKotlin(option *ConstOption) {
...
func (obj *Object) genKotlin(prefix string, option *ConstOption, builder *strings.Builder) {
	for idx, constObj := range obj.Children {
		...
		if constObj.objType == TypeEnumValue && !(obj.enumToPrefix || option.enumToPrefix) {
			...
			str = "\n" + option.getTabWidth() + "const val " + option.genIdentNameObj(prefix, constObj) + ": " + kotlinTypo(constObj.Typo) + " = " + value
		}
...
func (obj *Object) genKotlinStr(prefix string, option *ConstOption, builder *strings.Builder) {
	for _, constObj := range obj.genObjs {

		str := "\n" + option.getTabWidth() + "const val " + option.genIdentNameObj(prefix, constObj) + ": String = " + constObj.Value

		builder.WriteString(str)
	}
}

func (obj *Object) genKotlinId(prefix string, option *ConstOption, builder *strings.Builder) {
	for _, constObj := range obj.genObjs {

		str := "\n" + option.getTabWidth() + "const val " + option.genIdentNameObj(prefix, constObj) + ": " + kotlinTypo(constObj.Typo) + " = " + constObj.Value

		builder.WriteString(str)
	}
}

func (obj *NameSpace) genKotlin(option *ConstOption, builder *strings.Builder) {
	namespace := obj.Ident
	...
	if obj.parent == nil {
		if option.rootNamespace {
			...
			if obj.namespaceToPrefix || option.namespaceToPrefix {
				...
				builder.WriteString(option.getEnterCount(1) + option.getTabWidth() + "object " + option.getIdentNameObj(rootName, &obj.Object) + " {")
				option.level++
			}
		}
	} else {
		...
		if obj.namespaceToPrefix || option.namespaceToPrefix {
			...
			builder.WriteString(option.getEnterCount(len(obj.CommentDoc)) + option.getTabWidth() + "object " + option.getIdentNameObj(namespace, &obj.Object) + " {")
			option.level++
		}
	}
	...
	// generate consts
	obj.Object.genKotlin(prefix, option, builder)
	...
	for _, enumObj := range obj.Enums {
		...
		if enumObj.enumToPrefix || option.enumToPrefix {
			...
			builder.WriteString(option.getEnterCount(len(enumObj.CommentDoc)) + option.getTabWidth() + "enum class " + option.getIdentNameObj(enumName, enumObj) + "(val value: " + kotlinTypo(enumObj.Typo) + ") {")
			option.level++
