...
import (
	...
	parser "github.com/cymfarcat/constmaker/parser/const"
...
type ConstParser struct {
	...
	option        *src.Options
	objectStack   []*ConstObject
	currentObject *ConstObject
}

func NewConstParser(option *src.Options) *ConstParser {
	parser := &ConstParser{}
	parser.option = option

	parser.initialize(nil)
	parser.objectStack = make([]*ConstObject, 0)

	// push this to stack
	parser.pushStack(&parser.ConstObject)
...
func (obj *ConstParser) pushStack(object *ConstObject) {
	obj.objectStack = append(obj.objectStack, object)
}

func (obj *ConstParser) popStack() *ConstObject {
...
func (obj *ConstParser) topStack() *ConstObject {
	if len(obj.objectStack) == 0 {
...
func (obj *ConstParser) EnterConst(ctx *parser.ConstContext) {
	...
	constObj := NewConstObject(topStack, TypeObj)
...
func (obj *ConstParser) EnterEnum(ctx *parser.EnumContext) {
	topStack := obj.topStack()
	enum := NewConstObject(topStack, TypeEnum)
...
func (obj *ConstParser) EnterNamespace(ctx *parser.NamespaceContext) {
	...
	obj.pushStack(&namespace.ConstObject)
	obj.currentObject = &namespace.ConstObject
}
...
func (obj *ConstParser) EnterEnumValue(ctx *parser.EnumValueContext) {
	topStack := obj.topStack()
	enumValue := NewConstObject(topStack, TypeEnumValue)
...
func (obj *ConstParser) EnterIdentAssignValue(ctx *parser.IdentAssignValueContext) {
	...
	obj.currentObject = NewConstObject(obj.currentObject, TypeProperty)
}
...
func (obj *ConstParser) EnterTripleComment(ctx *parser.TripleCommentContext) {
	obj.currentObject.CommentTriple = ctx.GetText()
}
